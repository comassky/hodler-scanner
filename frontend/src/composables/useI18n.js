...
const messages = {
  en: {
    ...
    backtest: {
      ...
      bandsGuide: 'Read it against the dashed reference line: a band ABOVE it beat buying at a random time — historically a better entry. A band BELOW it did worse than buying anytime.',
      verdictLabel: 'Your goal — good moments to buy & hold',
      verdict_good: 'On this ticker, buying on Strong/Accumulate scores beat buying at a random time by {delta} over {h}. The score has helped time entries here.',
      verdict_neutral: 'On this ticker, buying on Strong/Accumulate scores performed about the same as buying at a random time ({delta} over {h}). No clear timing edge here.',
      verdict_bad: 'On this ticker, buying on Strong/Accumulate scores did worse than buying at a random time ({delta} over {h}). The score did not help time entries here — being positive isn\'t enough, it must beat the baseline.',
      verdictNoData: 'Not enough history at this horizon to judge whether the score helps.',
  ...
  fr: {
    ...
    backtest: {
      ...
      bandsGuide: 'À lire par rapport à la ligne de référence (pointillés) : un palier AU-DESSUS a battu un achat à un moment au hasard — historiquement une meilleure entrée. Un palier EN DESSOUS a fait moins bien qu’acheter n’importe quand.',
      verdictLabel: 'Ton objectif — bons moments pour acheter et garder',
      verdict_good: 'Sur ce titre, acheter sur les scores Fort/Accumuler a battu un achat à un moment au hasard de {delta} sur {h}. Le score a aidé à mieux timer les entrées ici.',
      verdict_neutral: 'Sur ce titre, acheter sur les scores Fort/Accumuler a fait à peu près comme un achat au hasard ({delta} sur {h}). Pas d’avantage de timing net ici.',
      verdict_bad: 'Sur ce titre, acheter sur les scores Fort/Accumuler a fait moins bien qu’un achat au hasard ({delta} sur {h}). Le score n’a pas aidé à timer les entrées ici — être positif ne suffit pas, il faut battre la référence.',
      verdictNoData: 'Pas assez d’historique à cet horizon pour juger si le score aide.',
